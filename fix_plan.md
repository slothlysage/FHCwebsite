...
- [ ] **0.3 Lint, format, and the `verify` script**
  Deps: 0.2.
  ESLint (next/core-web-vitals + @typescript-eslint), Prettier, and
  `"verify": "npm run lint && npm run typecheck && npm run test:coverage && npm run build"`.
  AC: `npm run verify` runs all four and exits non-zero if any fail.
...
- [ ] **2.7 Cart**
  ...
  AC: a cart containing a product whose price later changes reflects the *new*
