...
// order + order items (repos/orders' existing one-transaction insert),
// decrements inventory for each line, and empties the cart it paid for.
//
// specs/05-payments.md's "Transactions" section calls for all four of these
// writes (order, items, inventory movements, cart-clear) to commit or roll
// back together in ONE database transaction — that hardening, plus the
// explicit partial-failure rollback test it requires, is fix_plan.md task
// 3.5. This version satisfies 3.4's own AC ("replaying the same event twice
// creates exactly one order and decrements inventory once") via the
// webhook's event-id idempotency guard (src/lib/stripe/webhook.ts), which
// guarantees this function runs at most once per Stripe event — not yet via
// a single atomic transaction across all four writes here.
